...
from configs import dtcwt_config
from model import wpt_net
from dataset import dtcwt_transforms, dct_transforms
from dataset import wpt_transforms
from utils.statistics import ProgressMeter, AverageMeter, accuracy
from utils.utils import *

parser = argparse.ArgumentParser(description="DTCWT")
...
parser.add_argument('--log_desc_prefix', type=str, default='test_dtcwt')
...
parser.add_argument('--dtcwt_level', type=int, default=2)
parser.add_argument('--dtcwt_config_index', type=int, default=0)
parser.add_argument('--use_attention', type=str_to_bool, default=False)
parser.add_argument('--use_gate', type=str_to_bool, default=False)
...
def main():
    ...
    input_channels = 3 * (1 + 6 * 2 * config.dtcwt_level)
    input_channels = dtcwt_config.config_list[config.dtcwt_config_index]['num_channels']
    # TODO: DTCWT Mask
    # wpt_mask = np.array(config.wpt_mask, dtype=np.bool)
    # if isinstance(wpt_mask, np.ndarray):
    #     input_channels = wpt_mask.sum()

    mean = dtcwt_config.mean_std_list[config.dtcwt_config_index]['mean']
    std = dtcwt_config.mean_std_list[config.dtcwt_config_index]['std']

    # can use the same network with wpt net
    ...
    transform_class = dtcwt_config.config_list[config.dtcwt_config_index]['class']
    ...
    train_transform = transforms.Compose([
        ...
        transform_class(nlevels=config.dtcwt_level),
        # dtcwt_transforms.SimpleDTCWT(nlevels=config.dtcwt_level),
        # wpt_transforms.WPTMask(wpt_mask),
        # dtcwt_transforms.DTCWTToTensor(),
    ...
    train_dataset = dataset_cv2.ImageFolderCustom(train_dir, loader='opencv', transform=train_transform, num_cls=config.num_classes, half_data=False)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.batch_size, pin_memory=True, shuffle=True, num_workers=config.num_workers)

    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=config.batch_size, pin_memory=True, shuffle=False, num_workers=config.num_workers)
...
def adjust_learning_rate(optimizer, epoch, iteration, num_iter, warmup=False):
    ...
    max_iter = config.num_epochs * num_iter
